pipeline {
	...
	stages {
		stage('Checkout') {
			steps {
				...
				checkout([
					...
					submoduleCfg: [],
					userRemoteConfigs:[[
						credentialsId: 'GitHub_fmarinCeiba',
						url: 'https://github.com/fmarinCeiba/ceiba-adn.git'
					]]
				])
			}
		}
		...
		stage('Build project') { 
			steps { 
				echo "------------>Build project<------------"
				sh 'gradle --b ./cafe-internet-api/build.gradle clean'
				sh 'gradle --b ./cafe-internet-api/build.gradle build'
			}
		}
	...
	post {
		...
		success {
			...
			junit 'cafe-internet-api/build/test-results/test/*.xml'
		}
		failure {
			echo 'This will run only if failed'
			mail( to: 'francisco.marin@ceiba.com.co' ,
